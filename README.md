...
The below command will run the docker image `pedrotoliveira/lamp:latest`, creating a `mysql/` folder within your project. This folder will be linked to `/var/lib/mysql` where all of the MySQL files from container lives. You will now be able to stop/start the container and keep your database changes.
...
The below example can be added to your `~/.bash_profile` file to add the alias commands `ldi` and `launchdocker`. By default it will launch the 16.04 image - if you need the 14.04 image, simply change the `docker run` command to use `pedrotoliveira/lamp:latest-1404` instead of `pedrotoliveira/lamp:latest`.
```bash
# A helper function to launch docker container using pedrotoliveira/lamp with overrideable parameters
#
...
docker run -p "3000:80" pedrotoliveira/lamp:latest-1404 -d
...
[logo]: https://cdn.rawgit.com/pedrotoliveira/docker-lamp/831976c022782e592b7e2758464b2a9efe3da042/docs/logo.svg
